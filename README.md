...
In the 1980's, computer monitors across the world were mostly the same size. Screens were black with neon green text on them. This was great for basic HTML, where universities like Stanford, Harvard, and MIT could send disertations and research papers between each other. The paper's title got an H1, the author's names got an H2, and the actual content was in a Paragraph/P tag. Text was the most important thing, with ideas like positioning and resizing not really crucial for web development at that point.
...
As we entered the 2000's, we saw the rise of incredible technological advances. The computer became available to everyone, laptops allowed all of the power of a PC with full portability, and the rise of the Smartphone meant that we could all use Google, Facebook, and Youtube with a device that fit into our pockets. Online gaming meant that larger monitors became popular, while smartphones meant that the size of screens was about to change drastically, getting smaller and smaller, and more imporantly, changing from version to version, and thus user to user.
